 ...
 * @dp 是否四舍五入，1不四舍五入
 */
export function toFixed (num, s = 2, dp) {
    return BigNumber(num).toFixed(s, dp)
}
...
// 格式化输入框内容，限制只能输入数字和限制小位数
export function formatInputNumber (e, digits, min, max) {
    ...
    // 限制最小输入
    if (!isEmpty(min) && Number(newVal) < Number(min)) {
        newVal = min
    }
    // 限制最大输入
    if (!isEmpty(max) && Number(newVal) > Number(max)) {
        newVal = max
    }
...
// 判断非数字，非数字情况下返回0(0.00),数字情况下然后转换2位小数
export function fixedFormat (number, digit = 2) {
