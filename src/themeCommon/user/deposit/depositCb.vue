<template>
    <div class='pageWrap'>
        <LayoutTop :back='true' bottom-line :menu='false' :title="$t('wallet.wallet')" @back='onBack' />
...
import { isEmpty, localGet, localRemove, sessionGet } from '@/utils/util'
...
export default {
    setup (props) {
        ...
        // 处理回调参数
        const callbackUrl = sessionGet('callbackUrl')
        if (callbackUrl) {
            try {
                const { c, a, t } = JSON.parse(callbackUrl) || {}
                if (!currency && c) currency = c
                if (!accountId && a) accountId = a
                if (!tradeType && t) tradeType = t
            } catch {
            }
        }
