<template>
    <div class='pageWrap'>
        ...
        <div class='wrap'>
            ...
            <div class='receipt-info'>
                ...
                <div class='box' @click='showChannelPopup = true'>
                    <input disabled :placeholder="$t('pay8Account.bankSelect')" type='text' :value='channel.name' />
                ...
                <div class='box'>
                    <input v-model='account' :placeholder="$t('pay8Account.bankAccount')" type='text' />
    ...
    <!-- 提现通道弹窗 -->
    <van-popup v-model:show='showChannelPopup' position='bottom' round>
        <van-picker
            :cancel-button-text="$t('common.cancel')"
            :columns='channelList'
            :columns-field-names="{ 'text': 'name' }"
            :confirm-button-text="$t('common.sure')"
            @cancel='showChannelPopup = false'
            @confirm='selectChannel'
        />
    </van-popup>
    <!-- 资金密码弹窗 -->
    ...
    setup (props) {
        ...
        const queryBankList = () => {
            queryPayWithdrawBankList({
                ...
                if (res.check() && res.data.length > 0) state.channelList = res.data
                if (receiptInfo?.code) state.channel = state.channelList.find(el => el.code === receiptInfo.code) || {}
            })
        }

        // 选择银行卡通道
        const selectChannel = (elem, index) => {
            const item = state.channelList[index]
            state.channel = item
            state.showChannelPopup = false
        }
                ...
                if (res.check()) {
                    ...
                    if (!res.data.customerGroupEnable) {
                        return Dialog.confirm({
                            ...
                            if (onlineServices.value) { window.open(onlineServices.value) }
                        })
                    } else {
                        // 检测取款是否需要kyc
                        checkKyc()
        ...
        const confirm = () => {
            ...
            if (isEmpty(state.channel)) {
                return Toast(t('pay8Account.bankSelect'))
            }
            if (isEmpty(state.name)) {
                return Toast(t('pay8Account.inputName'))
            }
            if (isEmpty(state.account)) {
                return Toast(t('pay8Account.bankAccount'))
            }
