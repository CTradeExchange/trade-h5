...
import { createListByPlans, diffPriceString, fillProductLabel, handlerDealLastPrice, priceToPip } from './storeUtil.js'
import BigNumber from 'bignumber.js'

const IS_PRD = location.host === 'www.headline.net'
...
export default {
    ...
    state: {
        ...
        productList: [], // 产品列表
        productMap: {}, // 产品列表
        symbolCodeMap: {}, // 产品的symbolCode+tradeType作为key列表
        planMap: {}, // 管理所有玩法及每个玩法下的所有产品
        productActivedID: sessionGet('productActivedID'), // 当前操作的产品ID
        // 设置默认显示比特币，pre环境是6318，prd环境1798
        bitcoinId: IS_PRD ? '1798' : '8996',
        ...
        quoteListType: 'normal', // localGet('quoteListType') || 'block', // 切换产品列表的样式， block 块列表， normal 普通列表v√
    },
    getters: {
        // 用户自选列表
    ...
    mutations: {
        ...
        // 更新symbolCodeMap
        Updata_symbolCodeMap (state, data) {
            const productList = state.productList
            const symbolCodeMap = state.symbolCodeMap
            data.forEach(({ symbolCode, symbolId, tradeType }) => {
                if (symbolCode && productList.find(item => item.symbolId === symbolId)) {
                    const symbolKey = `${symbolId}_${tradeType}`
                    // prd部分symbolCode有斜杠
                    const symbolCodeKey = `${symbolCode.replace('/', '')}_${tradeType}`
                    vue_set(symbolCodeMap, symbolCodeKey, symbolKey)
                }
            })
        },
        // 更新玩法列表
        ...
        Update_product (state, data = {}) {
            ...
            if (!product.sell_price) data.sell_price_diff = []
            if (!product.buy_price) data.buy_price_diff = []
            assign(product, data)
        ...
        Update_productTick (state, dataArr = []) {
            ...
            dataArr.forEach(data => {
                ...
                if (!product.buy_price_pre) { // 缓存上一口价的裸行情
                    vue_set(product, 'buy_price_pre', product.buy_price)
                    vue_set(product, 'sell_price_pre', product.sell_price)
                    vue_set(product, 'cur_price_pre', product.cur_price)
                    vue_set(product, 'yesterday_close_price', product.yesterday_close_price)
                    vue_set(product, 'price_digits', product.price_digits)
                    vue_set(product, 'cur_color', 'grayColor')
                    vue_set(product, 'buy_color', 'grayColor')
                    vue_set(product, 'sell_color', 'grayColor')
                }
                // 计算涨跌
                product.cur_color = BigNumber(product.cur_price).eq(product.cur_price_pre) ? product.cur_color : BigNumber(product.cur_price).lt(product.cur_price_pre) ? 'fallColor' : 'riseColor'
                product.buy_color = BigNumber(product.buy_price).eq(product.buy_price_pre) ? product.buy_color : BigNumber(product.buy_price).lt(product.buy_price_pre) ? 'fallColor' : 'riseColor'
                product.sell_color = BigNumber(product.sell_price).eq(product.sell_price_pre) ? product.sell_color : BigNumber(product.sell_price).lt(product.sell_price_pre) ? 'fallColor' : 'riseColor'
                ...
                diffPriceString(product)

                // 更新上一口价的裸行情
                product.cur_price_pre = product.cur_price
                product.buy_price_pre = product.buy_price
                product.sell_price_pre = product.sell_price
            })
        },
        // 更新某个产品24H报价
        ...
        Update_quoteListType (state, type) {
            state.quoteListType = type
            localSet('quoteListType', type)
        },
        // 500ms之前的报价变成黑色
        Update_priceHighlight (state, data) {
            const product = state.productMap[data.symbolKey]
            product.sell_price_highlight = false
            product.buy_price_highlight = false
            product.cur_price_highlight = false
        }
    },
    actions: {
        ...
        setProductAllList ({ dispatch, commit, state, rootState, rootGetters, getters }) {
            ...
            if (symbolList.length) {
                const hasProductsKeys = Object.keys(planMap)
                const basePlans = rootState._base.plans
                let intersection
                const isWallet = rootState._base.wpCompanyInfo.isWallet // 现货玩法是否当钱包使用
                if (isWallet) {
                    intersection = basePlans.filter(v => hasProductsKeys.includes(v.tradeType) || Number(v.tradeType) === 5)
                } else {
                    intersection = basePlans.filter(v => hasProductsKeys.includes(v.tradeType))
                }
                ...
                let firstProductSymbolId
                if (firstTradeType && planMap[firstTradeType]) {
                    if (planMap[firstTradeType].includes(state.bitcoinId)) {
                        firstProductSymbolId = state.bitcoinId
                    } else {
                        planMap[firstTradeType][0]
                    }
                } else {
                    firstProductSymbolId = ''
                }
                // const firstProductSymbolId = firstTradeType && planMap[firstTradeType] ? planMap[firstTradeType][0] : ''
                const firstProductSymbolKey = firstProductSymbolId + '_' + rootState._base.tradeType
                commit('Update_productActivedID', sessionGet('productActivedID') || firstProductSymbolKey)
            }
            return planMap
        },
        // 产品基础信息列表
            ...
            if (state.symbolBaseLoaded === 0) {
                ...
                this.requestSymbolBase = findSymbolBaseInfoList(params).then((res) => {
                    if (res.check()) {
                        ...
                        commit('Updata_symbolCodeMap', res.data)
                    }
