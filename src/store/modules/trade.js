...
import { getTradeExchangeRate as _getTradeExchangeRate } from '@/api/fund'
import CheckAPI from '@/utils/checkAPI'
import { cachePendingParams } from './storeUtil.js'
import { minus, toFixed, plus, shiftedBy, pow, mul } from '@/utils/calculation'
import { vue_set, assign } from '@/utils/vueUtil.js'
import BigNumber from 'bignumber.js'
import { localSetJSON, localGetJSON } from '@/utils/util'
...
export default {
    ...
    state: {
        ...
        /** 不需要在图表展示持仓线的id */
        closeChartPositionLineId: localGetJSON('closeChartPositionLineId') || [],
        /** 按额下单或者按量下单，2按额 1按量 */
        entryType: 2,
        /** 货币对的交易汇率 */
        tradeExchangeRate: null,
        /** 显示产品搜索弹窗 */
    ...
    getters: {
        ...
        marketProfitLossRang (state, getters, rootState, rootGetters) {
            return (product, positionId) => {
                ...
                const curPosition = state.positionMap[positionId] // 当前修改的持仓
                const digits = product.price_digitss || product.symbolDigits
                const point = pow(0.1, digits)
                const pip = mul(point, product.pointRatio)

                /** 是否合约撮合 */
                const isMakingMatch = product.isMakingMatch
                let buy_price = state.pendingPrice['buy'] ? state.pendingPrice['buy'] : isMakingMatch ? product.rolling_last_price : product.buy_price
                let sell_price = state.pendingPrice['sell'] ? state.pendingPrice['sell'] : isMakingMatch ? product.rolling_last_price : product.sell_price

                if (curPosition) {
                // 如果当前是修改持仓，公式里面的价格则是：买方向取卖价、卖方向取买价
                    buy_price = sell_price = curPosition.direction === 1 ? product.sell_price : product.buy_price
                    if (isMakingMatch) {
                    // 合约撮合,止盈价和止损价范围根据最新价来计算
                        buy_price = product.rolling_last_price
                        sell_price = product.rolling_last_price
                    }
                }
        ...
        pendingPriceRang (state, getters, rootState) {
            ...
            if (!product) return EmptyPendingPriceRang
    ...
    mutations: {
        ...
        Update_tradeExchangeRate (state, data) {
            state.tradeExchangeRate = data
        },
        Update_closeChartPositionLineId (state, data) {
            state.closeChartPositionLineId = data
            localSetJSON('closeChartPositionLineId', data)
        },
        Update_entryType (state, data) {
        ...
        Update_positionList (state, { tradeType, list }) {
            ...
            list && list.forEach(item => {
                ...
                if (curPosition) Object.assign(item, { profitLoss: curPosition.profitLoss, previewStopPrice: curPosition.previewStopPrice }) // 如果map数据中已经有此持仓信息，将之合并到item
        ...
        Update_productSearchVisible (state, data) {
            state.productSearchVisible = data
        }
    },
    actions: {
        // 查询持仓列表
        ...
        // 预埋单列表
        queryPBOOrderPage ({ dispatch, commit, state, rootState, rootGetters, getters }, params) {
            if (!params.tradeType) return false
            cachePendingParams(params, pendingsConfig) // 缓存请求参数
            const accountListLen = rootState._user.customerInfo?.accountList?.length
            const tradeType = parseInt(params.tradeType)
            const dealMode = params.dealMode || getters.product?.dealMode
            delete params.dealMode
            ...
            } else if (Number(tradeType) === 5 || (Number(tradeType) === 2 && dealMode === 6)) {
                // Number(tradeType) === 2 && dealMode === 6 如果【合约逐仓】玩法时，根据当前产品的“成交模式==撮合”时，显示的是接了新接口的数据的【当前订单】
                if (Number(tradeType) === 2) {
                    pendingsConfig[tradeType].bizTypes = [13, 14]
                    // pendingsConfig[tradeType].bizType = 13
                }
        ...
        tradeRecordList () {},
        /** 查询货币对的交易汇率 */
        getTradeExchangeRate ({ commit }, params) {
            return _getTradeExchangeRate(params).then((res) => {
                if (res.check()) {
                    commit('Update_tradeExchangeRate', res.data)
                }
                return res
            })
        }
