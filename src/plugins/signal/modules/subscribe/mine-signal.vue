<template>
    <div v-if='cacheList.length' class='signal-more signal-more-sticky' @click='mergeData'>
    ...
    >
        <signal-item-th :item-type='5' />
        <virtual-list
            :key='listId'
            ref='listRef'
            class='signal-overflow-y signal-warp signal-warp-bg mine-signal-list'
...
export default {
    ...
    setup (props) {
        ...
        const loadData = () => {
            if (state.loadStatus === 'finished' || state.isLoading) return
        ...
        // 埋点我的信号页面有订阅记录的人
        watch([
            () => store.state._signal.isLoadSubscribe,
            () => store.state._signal.subscribeList,
            () => state.isGaSend
        ], (newVal) => {
