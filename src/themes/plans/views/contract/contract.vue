<template>
    <div class='contractWrapper'>
        <LayoutTop :back='true' :menu='false' :title="$t('information.reference')">
            <template #right>
                <img alt='connect' class='w-9 h-9' src='@/themeCommon/image/connect.png' @click='visitHelp' />
            </template>
        </LayoutTop>
        ...
        <div class='container'>
            ...
            <van-cell v-if='!product.isMakingMatch' :title="$t('contract.spread')" :value="$t('contract.float')" />
            <van-cell :title="$t('contract.singleTradeMargin')" :value="product.singleMinAmount + '-' + product.singleMaxAmount" />
            <van-cell
                v-if='!product.isMakingMatch'
                :title="$t('contract.limitDistance')"
                :value="product.priceMinLimit + '/' + product.priceMaxLimit + $t('contract.point')"
            />
            <van-cell
                v-if="product.marginInfo?.type === '1' && usedMarginSet && usedMarginSet.length"
                class='yfk'
                :title="$t('contract.advance')"
            >
            ...
            <van-cell
                :title="$t('contract.feeType')"
                :value="['1', '3'].includes(product.feeFormula) ? $t('contract.ratio') : $t('contract.amount')"
            />
...
export default {
    setup () {
        ...
        const interest = computed(() => {
            return (
                mul(product.value.buyInterest, 100) + '%/' + mul(product.value.sellInterest, 100) + '%(' + t('contract.interestRate') + ')'
            )
        })
        // 手续费
        const fee = computed(() => {
