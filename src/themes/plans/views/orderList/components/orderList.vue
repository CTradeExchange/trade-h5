<template>
    ...
    <div class='list-wrap'>
        <noAssetsTips v-if='recordList?.length === 0' :description="$t('withdraw.noRecords')" />
        <van-list
            :finished='finished'
    ...
    <!-- 切换产品 -->
    <ProductSearchSwipeModal v-if="$route.path.indexOf('/list') !== -1" :on-select='onSelectProduct' />
</template>
...
export default {
    ...
    setup (props) {
        ...
        const onSelectProduct = (p) => {
            ...
            store.commit('_trade/Update_productSearchVisible', false)
            resetParams()
        ...
        const openProductDropdown = (val) => {
            if (state.productVal === 1) { store.commit('_trade/Update_productSearchVisible', true) }
        }
