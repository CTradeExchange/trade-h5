<template>
    <div ref='productListEl' class='productListWrap'>
        <productItem
            ...
            :key='index'
            :product='productMap[item.symbolKey]'
...
import { debounce, getPathParams, isMobile } from '@/utils/util'
...
export default {
    ...
    props: ['productList', 'tradeData', 'showTrade'],
    setup (props) {
        ...
        const openProduct = (data) => {
            if (isUniapp) {
                ...
                router.push({
                    name: 'Order',
                    query: {
                        name: data?.symbolName?.replace(/ /g, '_'),
                        symbolId: data.symbolId,
                        tradeType: data.tradeType
                    }
                })
