            ...
            <van-swipe-item v-for='(item,i ) in swiperList' :key='i'>
                <div class='products-wrap'>
                    ...
                    >
                        ...
                        <klineSvg :data='klineData.slice(0,50)' :height='30' :product='product' :width='90' />
                    </div>
                </div>
            </van-swipe-item>
        </van-swipe>
...
export default {
    ...
    setup (props) {
        ...
        const subscribeData = computed(() => products.map(item => {
            return {
        ...
        // onMounted(() => {
        if (!h5Preview && subscribeData.value.length > 0) {
            QuoteSocket.batchGetKlineData(subscribeData.value)
        }
        // })
