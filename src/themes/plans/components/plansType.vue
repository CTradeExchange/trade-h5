<template>
    <!-- <van-popover v-if="newUI" v-model:show="show" :actions="plansList" @select="onSelect"> -->
    <van-popover v-if='newUI' :actions='plansList' @select='onSelect'>
        <template #reference>
            <!-- <div @click="show = ture"> -->
            <div class='h-full flex items-center'>
                <span class='iconRight'>
                    {{ plansList.find(i => i.id == current)?.text }}
                </span>
                <!-- <van-icon name="arrow-down" /> -->
            </div>
        </template>
    </van-popover>
    <div v-else class='playType'>
