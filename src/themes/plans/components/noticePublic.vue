...
import { getNoticePop } from '@/api/user'
...
export default {
    setup () {
                ...
                .then((res) => {
                    if (res.check()) {
                        ...
                        if (res.data && res.data.length > 0) {
                            ...
                            var nData = JSON.parse(localGet('noticeParams')) // 提取最新的pubTime
                            if (
                                customInfo ? customInfo.value.customerNo : false
                            ) {
                                // 不是
                                // console.log(nData)
                                if (customInfo.value.customerNo) {
                            ...
                            localSet(
                                'noticeParams',
                                JSON.stringify(noticeParams)
                            )
                        }
                        store.commit('_user/Update_currentPopup', popupStatus ? 'public' : 'drainage')
                    }
                })
                .catch((err) => {
        ...
        watch(() => [state.publicShow, currentPopup.value], () => {
            if (!state.publicShow && currentPopup.value === 'public') {
