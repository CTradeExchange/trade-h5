...
import { localGet, localSet, getQueryVariable, getQueryString, localGetJSON } from '@/utils/util'
import CheckAPI from '@/utils/checkAPI'
import Signal from '@/plugins/signal/store/signal'
...
export default createStore({
    ...
    state: {
        ...
        chartColorType: getQueryString('chartColorType') || Number(localGetJSON('CHART_CONFIG', {})?.chartColorType) || 1, // 涨跌颜色类型 1 绿涨红跌 2 红涨绿跌
    },
    getters: {
        ...
        companyCountryList (state, getters) {
            const companyCountry = state.companyCountry || ''
            const companyCountryArr = companyCountry.split(',')
            const result = state.countryListAll.filter(el => companyCountryArr.find(o => o === el.code))
            result.sort((a, b) => {
    ...
    mutations: {
        ...
        // 更新红涨绿跌类型
        Update_chartColorType (state, data) {
            state.chartColorType = data
        }
    },
    actions: {
        // 获取国家验区号
