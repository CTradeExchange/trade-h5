    ...
    <div class='page-wrap'>
        ...
        <div class='conditon-wrap'>
            <div v-for='(item,index) in elementList' :key='index' class='condition-item'>
                <div v-if="item.showType === 'input'" class='field'>
                    <div class='field-label'>
                        {{ item.elementName }}
                    </div>
                    <el-input v-model.trim='conditionModel[item.elementCode]' :clearable='true' :placeholder='$t("common.input")+item.elementName' :type='item.elementCode === "phone" ? "number" : "text"' />
                </div>
                <!-- <van-field
                ...
                <div v-if="item.showType === 'inputGroup'">
                    <div class='field'>
                        ...
                        <el-select v-model='typeValue' clearable :placeholder='$t("register.chooseCertificateType")' :style="{ width:'100%' }" @change='changeType'>
                            <el-option v-for='(o,i) in columns' :key='i' :label='o.name' :value='o' :value-key='o.code' />
                        </el-select>
                    </div>
                    <div class='field'>
                        <div class='field-label'>
                        ...
                        <el-input v-model.trim='conditionModel[typeCode]' :placeholder="$t('register.pleaseEnter')+ typeValue" />
                    </div>
        ...
        <!-- <div v-if='!hideSubmitBtn' class='footer-btn' @click='onConfirm'>
            <span> {{ $t('common.submit') }}</span>
            <el-button   class='confirm-btn' size='large' @click='onConfirm'>
                {{ $t('common.submit') }}
            </el-button>
        </div> -->
...
export default {
    ...
    setup (props) {
        ...
        const getInputGroupData = () => {
            getListByParentCode({ parentCode: 'id_card_type' }).then(res => {
                if (res.check() && res.data.length > 0) {
                    state.columns = res.data.filter(o => o.code !== 'drving_license') // 去除驾驶证的选项
        ...
        const onConfirm = () => {
            ...
            // firebase
            firebaseAnalytics('kyc_write_submit', {
                key: 'kyc_write_submit',
                value: 'submit',
            })
            let params
            /* 具体业务的kyc认证 */
