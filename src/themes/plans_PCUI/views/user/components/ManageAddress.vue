<template>
    <div class='address-body'>
        <div class='fp-content'>
            <div class='fp-list'>
                <div v-if='addressList.length > 0' class='fp-list-box'>
                    ...
                    >
                        <div :class="['list-address-item', !chainNameList.find(k=>k.name==item.chainName) ? 'not-erc20' : '']">
                            <img
                                v-if='!chainNameList.find(k=>k.name==item.chainName)'
        ...
        <div class='fp-action'>
            ...
            <div class='fp-confirm' @click='handleSubmit'>
                {{ $t('withdraw.confirm') }}
            </div>
        </div>
    </div>
...
export default {
    ...
    setup (props, { emit }) {
        ...
        const selectWallet = (item) => {
            // 目前仅支持erc20,其他的禁止选择
            if (!props.chainNameList.find(k => k.name == item.chainName)) {
                return
            }
        ...
        const readChainsName = () => {
            if (props.chainNameList) {
                const strArr = []
                props.chainNameList.forEach(item => {
