<template>
    <DrawPage
        ...
        @onBack="$router.replace($route.path.replace('/withdrawCoin', ''))"
        @toPage="$router.replace($route.path.replace('withdrawCoin', 'withdrawRecord'))"
    >
        <div class='container'>
            <!-- 加载中组件 -->
            ...
            <div class='fund'>
                ...
                <div class='fund-input'>
                    ...
                    <router-link
                        ...
                        class='href'
                        :to="`${$route.path.replace('withdrawCoin', 'setFundPwd')}`"
                    >
                        {{ $t("login.goSet") }}
                    </router-link>
...
export default {
    ...
    setup (props, { emit }) {
        ...
        const { currency, accountId, currentTab } = route.query
        const tradeType = Number(route.query?.tradeType) || 5
                ...
                if (Number(res.data) !== 2) {
                    return Dialog.alert({
                        ...
                        if (customInfo.openAccountType === 0) {
                            router.replace({
                                path: route.path.replace('withdrawCoin', 'authentication'),
                    ...
                    if (!withdrawConfig.accountActiveEnable) {
                        return Dialog.confirm({
                            ...
                            .then(() => {
                                router.replace({
                                    path: route.path.replace('withdrawCoin', 'depositChoose'),
                                    query: {
                                        tradeType,
                                        accountId,
        ...
        const goWalletAdd = () => {
            ...
            router.replace({
                path: route.path.replace('withdrawCoin', 'walletAddress'),
