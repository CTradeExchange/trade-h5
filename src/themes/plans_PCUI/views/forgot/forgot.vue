    ...
    <div class='forgot'>
        ...
        <div class='container'>
            <div class='content'>
                ...
                <div v-if='type === "login"' class='to-login'>
                    {{ $t('login.remebered') }}
...
import { isEmpty, localGetJSON, getDefaultZoneIndex } from '@/utils/util'
            ...
            if (value[0] && value[0].length) {
                ...
                if (loginInfo?.accountType) {
                    if (loginInfo.accountType === 2) {
                        ...
                        //当路由携带了这个说明是从忘记密码跳转过来的就用路由的参数  
                        if(route.query?.loginNameType){
                            state.active=route.query.loginNameType=='email'?1:0
                            state.curTab = route.query.loginNameType=='email'?1:0
                        }else{
                            state.curTab = 0
                            state.active = 0
                        }
