<template>
    ...
    >
        ...
        <template #reference>
            <Popover :text='$t("cRoute.transfer")'>
                <template #reference>
                    <i
                        class='icon_zhuanhuan2 addIcon pc_trade_deposit_add_ga'
...
// 所有玩法
const allPlans = computed(() => store.state._base.allPlans)

// 当前主题
const theme = computed(() => store.state.theme)
const themeValue = theme.value === 'night' ? 'dark' : 'light'
...
const addIconHandler = () => {
    const walletTradeType = 8
    const { tradeType } = props.product
    const { currency } = props.account
    if (Number([1, 2].includes(tradeType))) {
        router.push({
            path: '/order/transfer',
            query: {
                tradeType: walletTradeType,
                toTradeType: tradeType,
                toCurrency: currency
            }
        })
    }
