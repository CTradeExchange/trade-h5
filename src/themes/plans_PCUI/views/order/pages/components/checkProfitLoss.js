...
export function profitLossPriceCompare (type, direction, price, profitStopRange, $t) {
    ...
    if (direction === 1) {
        if (type === 'stopLoss') {
            ...
            if (gt(price, max)) {
                return $t('trade.profitLossWarn', ['≤' + max])
            } else if (lt(price, min)) {
                return $t('trade.profitLossWarnTip')
            } else {
                return false
            }
        } else if (type === 'profit') {
            ...
            if (lt(price, min)) {
                return $t('trade.takeProfitWarn', ['≥' + min])
            } else if (gt(price, max)) {
                return $t('trade.takeProfitWarnTip')
            } else {
                return false
            }
        }
        ...
        if (type === 'stopLoss') {
            ...
            if (lt(price, min)) {
                return $t('trade.profitLossWarn', ['≥' + min])
            } else if (gt(price, max)) {
                return $t('trade.profitLossWarnTip')
            } else {
                return false
            ...
            if (gt(price, max)) {
                return $t('trade.takeProfitWarn', ['≤' + max])
            } else if (lt(price, min)) {
                return $t('trade.takeProfitWarnTip')
            } else {
                return false
