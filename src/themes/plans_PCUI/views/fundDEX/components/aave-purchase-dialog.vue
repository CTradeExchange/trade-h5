<template>
            ...
            <div class='popup-model'>
                ...
                <div v-if='showSuccess' class='success-content'>
                    <div class='success-block'>
                        ...
                        <p class='amount'>
                            You Purchased {{ count }} {{ fund.thirdCoinCode }}
                        </p>
                        <div class='add-box'>
                            <img class='coin' :src='"/images/tokens/"+ fund.thirdCoinCode.toLowerCase() +".svg"' />
                            <p class='des'>
                ...
                <div v-else class='model-content'>
                    <div class='block'>
                        ...
                        <div class='box'>
                            <div class='row-1'>
                                <input
                                    placeholder='0.00'
                                    :readonly='allowanceLoading || purchaseLoading'
                                    type='text'
                                    :value='count'
                                    @input='onInput'
                                />
                                <div class='coin'>
                                    <img :src='"/images/tokens/"+ fund.thirdCoinCode.toLowerCase() +".svg"' />
                            ...
                            <div class='row-2'>
                                ...
                                <strong>
                                    ...
                                    <span v-else>
                                        Balance {{ formatAmount(assetBalance, 5) }}
                                    </span>
                                    <button @click='onMax'>
                                        MAX
                                    </button>
                    ...
                    <div class='total'>
                        ...
                        <div v-else>
                            <div v-if='fee'>
                                ...
                                <van-popover v-model:show='showTotalPopover' placement='top'>
                                    <div class='popover-content'>
                    ...
                    <div class='handle-action'>
                        ...
                        <!-- 未输入数量 -->
                        <button v-else-if='count <= 0 || allowanceBalanceLoading' class='disable'>
                            <van-loading v-if='allowanceBalanceLoading' :size='24' />
                            <span v-else>
                                Enter an amount
                            </span>
                        </button>
                        ...
                        <div v-else>
                            ...
                            <div v-if='showAllowance'>
                                ...
                                <button :class="allowanceLoading || isApproved ? 'disable' : 'default'" @click='depositApproveHandler'>
                            ...
                            <!-- 申购按钮 -->
                            <button :class="isPurchase && !purchaseLoading ? 'default' : 'disable'" @click='depositHandler'>
                                <label v-if='purchaseLoading'>
                                    <van-loading :size='24' />
                                    <span>Purchasing {{ fund.thirdCoinCode }}</span>
                                </label>
                                <span v-else>
                                    Purchase {{ fund.thirdCoinCode }}
                                </span>
    ...
    <!-- 授权提示弹窗 -->
    <aave-approve-tip v-model:show='showApproveTip' />
</template>
...
import { formatAmount, toFixed, pow, mul, div, limitNumber } from '@/utils/calculation'
...
const getAssetBalance = () => {
    ...
    if (isPurchaseETH.value) {
        ...
        getWalletBalance(fund.value.chainId, fund.value.assetId, fund.value.decimals).then(res => {
            console.log('获取代币余额成功', res)
            assetBalanceLoading.value = false
...
const allowanceHandler = () => {
    if (fund.value.chainId !== store.state._web3.chainId) return false
    allowance(fund.value.assetId).then(value => {
        allowanceBalance.value = div(value, pow(10, fund.value.decimals))
        allowanceBalanceLoading.value = false
        if (Number(count.value) > Number(allowanceBalance.value)) {
            showAllowance.value = true
        } else {
            showAllowance.value = false
        }
...
// 代币存款授权
const depositApproveHandler = () => {
    if (fund.value.chainId !== store.state._web3.chainId) return false
    if (allowanceLoading.value || isApproved.value) return
    allowanceLoading.value = true
    const approveValue = mul('999999', pow(10, 18))
    depositApprove(fund.value.assetId, approveValue).then(res => {
        console.log('代币存款授权成功', res)
        allowanceLoading.value = false
        isApproved.value = true
        approvedData.value = res
        rejectTip.value = ''
    }).catch(err => {
        console.log('代币存款授权失败', err)
        allowanceLoading.value = false
        showAllowance.value = true
...
const depositHandler = () => {
    if (fund.value.chainId !== store.state._web3.chainId) return false
    if (!isPurchase.value || purchaseLoading.value) return
    purchaseLoading.value = true
    const value = mul(count.value, pow(10, fund.value.decimals))
    if (isPurchaseETH.value) {
        depositETH(value).then(res => {
            purchaseLoading.value = false
            ...
            console.log('代币存款成功', res)
        }).catch(err => {
            purchaseLoading.value = false
            rejectTip.value = err.message
            console.log('代币存款失败', err)
        })
    } else {
        depositDAI(fund.value.assetId, value).then(res => {
            purchaseLoading.value = false
            showSuccess.value = true
            successData.value = res
            rejectTip.value = ''
...
const getPurchaseFee = () => {
    const value = mul(count.value, pow(10, fund.value.decimals))
    estimateFee(fund.value.assetId, value, unref(isPurchase)).then(res => {
        console.log('获取申购手续费成功', res)
        feeLoading.value = false
        fee.value = toFixed(res)
    }).catch(err => {
        console.log('获取申购手续费失败', err)
        feeLoading.value = false
    })
}
...
const addCoin = () => {
    if (fund.value.chainId !== store.state._web3.chainId) return false
    const imgUrl = location.origin + '/images/tokens/' + (fund.value.thirdCoinCode.toLowerCase()) + '.svg'
    addToken(fund.value.aTokenAddress, 'a' + fund.value.thirdCoinCode, fund.value.decimals, imgUrl).then(res => {
        console.log('添加代币到钱包成功', res)
    }).catch(err => {
        console.log('添加代币到钱包失败', err)
    })
}
...
watch(() => count.value, () => {
    if (isEmpty(count.value) || Number(count.value) <= 0) {
        ...
        if (isQueryBalance) allowanceBalanceLoading.value = true
...
const onInput = (e) => {
    ...
    if (Number(newval) > Number(assetBalance.value)) newval = assetBalance.value
...
const inputHandler = debounce(() => {
    ...
    // 查询授权余额
    if (isQueryBalance) allowanceHandler()
    // 获取申购手续费
    getPurchaseFee()
}, 500)
...
// 点击MAX
const onMax = () => {
    if (assetBalance.value <= 0) return
    count.value = assetBalance.value
}
