...
export default {
    ...
    setup (props, { emit }) {
        ...
        const columns = computed(() =>
            accountList.value
                .filter((item) => item.tradeType === Number(props.tradeType))
                .map((el) => {
                    return {
        ...
        const accountList = computed(() =>
            store.state._user.customerInfo.accountList.filter(
                (el) => Number(el.tradeType) === Number(props.tradeType)
            )
        )

        state.outCurrency = props.defaultCurrency || columns.value[0].currency
        state.accountId = props.defaultAccountId || columns.value[0].accountId
                    ...
                    if (res?.check()) {
                        ...
                        if (Number(res.data) !== 2) {
                            return Dialog.alert({
                                ...
                                router.push({
                                    path: route.path + '/authentication',
                                    query: {
                                        businessCode: 'withdraw'
                                    }
                                })
