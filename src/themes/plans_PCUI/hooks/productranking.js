...
export default (params) => {
    ...
    console.log('hook_quote/querySymbolBaseInfoList')
    store.dispatch('_quote/querySymbolBaseInfoList', null).then(res => {
        if (res.check()) {
            ...
            sortPlans.filter(plan=> planMap[plan]).forEach(plan=>{
                ...
                let symbolList = planMap[plan]?.symbolList || []
                symbolList = symbolList.filter(el=> productMap[`${el}_${plan}`]?.symbolName);   // 过滤出有权限的产品列表
