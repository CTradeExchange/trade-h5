    ...
    <div class='upgrade'>
        <div class='container'>
            ...
            <div>
                ...
                <p v-if='endTime' class='labelContent' v-html="$t('upgrade', { time: endTime, timezone })">
                </p>
            </div>
        </div>
    </div>
...
export default {
    data () {
        ...
        return {
            lang: getCookie('lang'),
            endTime: '',
            timezone: offsetInHours >= 0 ? `+${offsetInHours}` : offsetInHours
        }
    },
    mounted () {
        this.initLottie()
    ...
    methods: {
        ...
        getConfigSystem () {
            ...
            axios
                .get('https://cats2.oss-cn-hongkong.aliyuncs.com/configSystem/configSystem.json', {
                    params: {
                        timestamp: Date.now(),
                    },
                })
                .then(({ data }) => {
                    ...
                    this.endTime = dayjs(parseInt(data.endTime)).format('HH:mm')
                })
        },
    },
}
