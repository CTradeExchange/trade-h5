...
export function getDaiPrice (chainId, address = 'ethereum') {
    ...
    if (address.toLowerCase() !== 'ethereum') {
        url = `https://api.coingecko.com/api/v3/simple/token_price/${chain[chainId]}/?contract_addresses=${address}&vs_currencies=usd`
    }
    return request({
        url: url,
        method: 'GET',
...
export function getFundPrice (chainId, contract_id) {
    return request({
        url: `https://api.coingecko.com/api/v3/simple/token_price/${chain[chainId]}/?contract_addresses=${contract_id}&vs_currencies=usd`,
        method: 'GET',
        params: {}
    })
}
