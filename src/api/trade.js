    ...
    return request({
        ...
        if (res?.data?.length) {
            res.data = res.data.map(item => {
                // 是否合约撮合产品
                const isMakingMatch = item.dealMode === 6 && item.tradeType === 2
                return {
                    ...item,
                    isMakingMatch
                }
...
/* 取消预埋单 */
export function closePboOrder (data, extra = {}) {
    return request({
...
/* 仓位试算 */
export function positionCalculation (data) {
    return request({
        url: '/global/tradeapi.app.OrderApiService.positionCalculation',
        method: 'post',
        toastErr: false,
        headers: {
            group: 'tradeApi',
...
/* 检查订单状态(1=调整杠杆倍数，2=调整保证金，3=订单是否成交) */
export function checkOrderStatus (data) {
    return request({
        url: '/global/tradeapi.app.OrderApiService.checkOrderStatus',
        method: 'post',
        toastErr: false,
        headers: {
            version: '0.0.1',
