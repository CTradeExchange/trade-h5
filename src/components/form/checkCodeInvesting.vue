<template>
    <div class='checkCodeBar van-hairline--bottom'>
        <div class='checkCodeInput'>
            <input :id='id' class='input' v-bind='$attrs' :placeholder='label' type='text' :value='modelValue' @input='onInput' />
            <!-- <label v-if='label' class='label' :for='id'>
            {{ label }}
        </label> -->
        </div>
        <a v-if='clear' v-show='modelValue.length' class='van-icon van-icon-clear' href='javascript:;' @click='onClear'></a>
        <button id='getCodeButton' ref='getCodeBtn' class='getCodeBtn' :disabled='disabled' type='button' @click='getCode'>
            <van-loading v-if='loading' size='20px' />
            <span v-else id='getCodeButton1' class='getCodeBtnText' style='color: #000'>
                {{ getCodeText }}
            </span>
        </button>
