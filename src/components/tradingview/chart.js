import { isEmpty, localSet, localGet, localGetJSON } from '@/utils/util'
...
class Chart {
    constructor ({ containerId, initial, property = {}, indicators = [], extension = {}, disabled_features = [] }, chartReadyCallback = () => {}) {
        // 产品初始值
        this.initial = {
        ...
        this.interval = String(initial.interval || '1')
        this.buyPrice = '' // 买价
        this.sellPrice = '' // 卖价
        // 产品id
    ...
    _init () {
        ...
        // 获取横竖屏配置项
        const options = { ...this._options, disabled_features: [...this._options.disabled_features, ...this.disabled_features], }
        ...
        // 获取用户上次划线缓存
        const saved_data = localGetJSON('__TRADEVIEW_STATE__')
        const chartproperties = localGetJSON('tradingview.chartproperties')
        const getTimeZone = () => {
            try {
                return chartproperties?.timezone || dayjs.tz.guess()
            } catch (error) {
                return chartproperties?.timezone
            }
        }
        ...
        this.widget.onChartReady(() => {
            // 不在缓存中的指标数据直接删除
            this.widget.activeChart().getAllStudies().filter(item => !this.indicators.find(e => e?.name === item?.name)).forEach(e => {
    ...
    // 监听窗口变化
    _bindResize () {
        if (!this.extension.fullScreen || !window.isPC) return
    ...
    _setProperty (property, overrides = {}) {
        Object.keys(property || {}).forEach(key => {
            ...
            case 'downColor': {
                ...
                overrides['mainSeriesProperties.' + 'candleStyle' + '.upColor'] = property.upColor
                overrides['mainSeriesProperties.' + 'candleStyle' + '.downColor'] = property.downColor
                overrides['mainSeriesProperties.' + 'candleStyle' + '.wickUpColor'] = property.upColor
                overrides['mainSeriesProperties.' + 'candleStyle' + '.wickDownColor'] = property.downColor
                overrides['mainSeriesProperties.' + 'candleStyle' + '.borderUpColor'] = property.upColor
                overrides['mainSeriesProperties.' + 'candleStyle' + '.borderDownColor'] = property.downColor
                overrides['linetoolexecution.arrowBuyColor'] = property.upColor
                overrides['linetoolexecution.arrowSellColor'] = property.downColor
                break
            }
            case 'chartType': {
    ...
    setChartType = (chartType) => {
        if (typeof chartType === 'number' && !isNaN(chartType)) {
            this.widget.activeChart().setChartType(Number(chartType))
        }
    }
    ...
    // 实时tick
    setTick (price, time, volume, symbolId) {
        if (price === undefined || symbolId !== this.symbolId) return
        this.datafeed._historyProvider.onTick(price, time, volume, symbolId)
